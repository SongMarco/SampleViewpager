...
import static com.example.administrator.sampleviewpager.R.id.spinnerTimerSetting;
import static com.example.administrator.sampleviewpager.subSources.BasicInfo.KEY_ADDWO_MODE;
import static com.example.administrator.sampleviewpager.subSources.BasicInfo.MODE_MODIFY;
import static com.example.administrator.sampleviewpager.subSources.BasicInfo.MODE_VIEW;
...
public class AddWorkoutActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinnerTimer = (Spinner) findViewById(spinnerTimerSetting);
        spinnerNumOrTime = (Spinner) findViewById(R.id.spinnerNumOrTime);

        Intent intent = getIntent();

        strAddWoMode = intent.getStringExtra(KEY_ADDWO_MODE);


        //메모를 걍클릭 함(모드뷰), or 롱클릭 -> 수정누름 (MODE_MODIFY)
        // 기존 내용을 먼저 그려주고, 사용자의 입력을 저장해준다
        if (strAddWoMode.equals(MODE_MODIFY) || strAddWoMode.equals(MODE_VIEW)) {
